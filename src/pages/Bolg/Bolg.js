...
export const Blog = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=5a85e135dc7864482b9a3c9378e912b1`
    )
      .then((res) => res.json())
  ...
  return (
    <>
      <ul className="container blog-list">
        {data.length &&
          data.map((e) => (
            <li key={e.title} className="blog-item">
              <img  className="blog-img" src={`https://image.tmdb.org/t/p/w500${e.backdrop_path}`} alt="" />
